...
def procesar_flujo(item_id, pulse_name):
    ...
    try:
        ...
        # Se solicita la información de las columnas (para ver si hay factura) y los assets (archivos físicos)
        ...
        # --- LÓGICA DE VINCULACIÓN CON FILTRO CONTABLE ---
        facturas_ids = models.execute_kw(ODOO_DB, uid, ODOO_API_KEY, 'account.move', 'search', [[('invoice_origin', '=', order_name)]])
        
        # Verificamos si la columna 'archivo2' de Monday tiene algo escrito (texto o nombre de archivo)
        tiene_factura_monday = bool(cols_text.get('archivo2'))
        ...
        for f in assets:
            ...
            # 1. Siempre subir al Pedido de Venta
            ya_existe_venta = models.execute_kw(ODOO_DB, uid, ODOO_API_KEY, 'ir.attachment', 'search_count', [[('res_model', '=', 'sale.order'), ('res_id', '=', order_id), ('checksum', '=', f_hash)]])
            if not ya_existe_venta:
                print(f"📤 Vinculando {nombre_f} a Ventas (sale.order ID: {order_id})")
            ...
            # 2. Subir a Contabilidad SOLO SI la columna archivo2 tiene contenido
            if tiene_factura_monday:
                for f_id in facturas_ids:
                    ya_existe_factura = models.execute_kw(ODOO_DB, uid, ODOO_API_KEY, 'ir.attachment', 'search_count', [[('res_model', '=', 'account.move'), ('res_id', '=', f_id), ('checksum', '=', f_hash)]])
                    if not ya_existe_factura:
                        print(f"📤 Vinculando {nombre_f} a Contabilidad (account.move ID: {f_id})")
